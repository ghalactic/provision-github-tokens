...
- **Central token storage**: the provider retains issued tokens for later
  revocation. Creates a high-value attack target, since a single breach exposes
  every active token.
- **Encrypted secrets with a revocable gate key**: provision encrypted secrets
  ...
  every consumer to perform decryption, which excludes consumers like Dependabot
  that need plaintext tokens.
