...
it("resolves a list of account patterns into a list of accounts", async () => {
  ...
  expect(
    appRegistry.resolveAccounts(...["org-a", "user-b"].map(createNamePattern)),
  ).toEqual(["org-a", "user-b"]);
  expect(
    appRegistry.resolveAccounts(...["org-*"].map(createNamePattern)),
  ).toEqual(["org-a", "org-c"]);
  expect(
    appRegistry.resolveAccounts(...["user-*"].map(createNamePattern)),
  ).toEqual(["user-b"]);
  expect(appRegistry.resolveAccounts(...["*"].map(createNamePattern))).toEqual([
    "org-a",
    "user-b",
    "org-c",
  ]);
  expect(
    appRegistry.resolveAccounts(...["xxx"].map(createNamePattern)),
  ).toEqual([]);
});
