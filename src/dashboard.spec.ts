...
it("renders a failure dashboard for a secret that failed to create", async () => {
  ...
  await expect(toMarkdown(issue.body)).toMatchFileSnapshot(
...
it("renders a config issue dashboard for root-level errors", async () => {
  const issue = renderConfigIssueDashboard(testContext, {
    requester: { account: "account-x", repo: "repo-x" },
    configPath: ".github/ghalactic/provision-github-tokens.yml",
    error: new ValidateError("Invalid requester configuration", [
      {
        instancePath: "",
        schemaPath: "#",
        keyword: "type",
...
it("renders only the intro and run link when there are no results", async () => {
  const issue = renderFailureDashboard(
    testContext,
    [],
    [],
  ...
  expect(issue.title).toBe(FAILURE_DASHBOARD_TITLE);
  await expect(toMarkdown(issue.body)).toMatchFileSnapshot(
    join(fixturesPath, "failure-dashboard-no-results.md"),
  );
});

it("renders a config issue dashboard for a non-validation error", async () => {
  const issue = renderConfigIssueDashboard(testContext, {
    requester: { account: "account-x", repo: "repo-x" },
    configPath: ".github/ghalactic/provision-github-tokens.yml",
  ...
  expect(issue.title).toBe(CONFIG_ISSUE_DASHBOARD_TITLE);
  await expect(toMarkdown(issue.body)).toMatchFileSnapshot(
    join(fixturesPath, "config-issue-error.md"),
  );
});

it("renders a config issue dashboard for invalid YAML", async () => {
  let error: unknown;
  ...
  try {
    parseYaml(
      {},
      "a: [1, 2\nb: {c: 1\n",
      "account-x/repo-x/.github/ghalactic/provision-github-tokens.yml",
    );
  } catch (cause) {
    error = cause;
  }
  ...
  expect(issue.title).toBe(CONFIG_ISSUE_DASHBOARD_TITLE);
  await expect(toMarkdown(issue.body)).toMatchFileSnapshot(
    join(fixturesPath, "config-issue-yaml.md"),
  );
});

it("renders invalid YAML without parse errors", async () => {
  let error: unknown;

  try {
    // Valid syntax, but resolution fails, so the parser surfaces no errors.
    parseYaml(
      {},
      "a: *x\n",
      "account-x/repo-x/.github/ghalactic/provision-github-tokens.yml",
  ...
  expect(issue.title).toBe(CONFIG_ISSUE_DASHBOARD_TITLE);
  await expect(toMarkdown(issue.body)).toMatchFileSnapshot(
    join(fixturesPath, "config-issue-yaml-unresolved.md"),
  );
});
...
it("omits token sections when authorization has no token result", () => {
  ...
  expect(toMarkdown(issue.body)).toContain("## Request authorization");
  expect(toMarkdown(issue.body)).not.toContain("## Token creation");
  expect(toMarkdown(issue.body)).not.toContain("### Token");
});
